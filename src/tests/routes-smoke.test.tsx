...
vi.mock("@/hooks/use-confirmable-action", () => ({
  useConfirmableAction: () => ({
    pendingConfirmation: { message: "Delete?", successText: "Deleted.", action: async () => undefined },
    isConfirmDialogOpen: confirmOpen,
    runAction: async (_successText: string, action: () => Promise<unknown>) => action(),
...
describe("route smoke", () => {
  it("renders meals route shell", () => {
    ...
    expect(screen.getByRole("heading", { name: /daily calories/i })).toBeTruthy()
    expect(screen.getByRole("checkbox", { name: /show archived meals/i })).toBeTruthy()
  ...
  it("renders people route shell", () => {
    ...
    expect(screen.getByRole("checkbox", { name: /show archived records/i })).toBeTruthy()
  ...
  it("renders manage route shell", () => {
    ...
    expect(screen.getByRole("heading", { name: /catalog management/i })).toBeTruthy()
    expect(screen.getByRole("checkbox", { name: /show archived records/i })).toBeTruthy()
